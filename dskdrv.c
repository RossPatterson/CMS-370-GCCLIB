...
static int reopen(FILE *theFile) {
    ...
    rc = CMSfileOpen(theFile->fileid, theFile->buffer, theFile->filemaxreclen,
                     theFile->fscb.format[0],
...
static int open(char filespecwords[][10], FILE *theFile) {
    ...
    if (theFile->access & ACCESS_TRUNCATE) {
        ...
        if (strlen(fmode) == 0)
            strcpy(fmode, "*"); /* Lets search all drives! */
        sprintf(fileid, "%-8s%-8s%-2s", fname, ftype, fmode);
        ...
        if (theFile->access & ACCESS_APPEND)
            recnum = existingFileState->numRecords + 1;
    ...
    sprintf(theFile->name, "%s %s %s", fname, ftype,
            fmode); /* copy in the filename that will be used in error msgs */
    ...
    switch (rc) {
        ...
        case 28:  /*  the file was not found */
            if (theFile->access & ACCESS_TRUNCATE)
                break;   /* file not found is OK if not appending */
            errno = ENOENT;
            free(theFile->buffer);
            theFile->buffer = NULL;
            return 0;
...
static int close(FILE *stream) {
    ...
    if (rc == 6)
        rc = 0; /* Ignore not open error - it means not read/writen yet */
    if (stream->buffer) {
        free(stream->buffer);
        stream->buffer = 0;
    }
...
static int write(FILE *file) {
    ...
    if ((file->fscb.format[0] == 'F') &&
        (file->reclen < file->filemaxreclen)) { /* PAD */
        if (file->access & ACCESS_TEXT) pad = ' ';
        else pad = 0;
        memset(file->buffer + file->reclen, pad,
    ...
    if (file->access & ACCESS_APPEND)
        file->fscb.recordNum = 0;         /* appending, so start at next record */
...
/* Driver Strusture                                                                               */
/**************************************************************************************************/
CMSDRIVER dskiodrv = {open, close, getpos, getend, setpos, write, read,
                      postread};
